...
namespace Controllers.Player
{
    public class PlayerParticleController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            currentParticleExplode = Instantiate(particleExplode, manager.transform.position,
                particleExplode.transform.rotation);
            currentParticle.Stop();
            currentParticleExplode.Stop();
            //       currentParticle.gameObject.SetActive(false);
        }

        public void StartParticle(Transform instantiateTransform)
        {
            ...
            currentParticleExplode.gameObject.transform.position = new Vector3(instantiateTransform.position.x,
                instantiateTransform.position.y + 5, instantiateTransform.position.z);
